...
import com.ktdsuniversity.edu.exceptions.UserIdendifyNotMatchException;
...
public class MemberServiceImpl implements MemberService {
	...
	public MemberVO getMember(MemberVO memberVO) {
		...
		if (salt == null) {
			throw new UserIdendifyNotMatchException(memberVO, "아이디 또는 비밀번호가 일치하지 않습니다");
		}
		...
		if (member == null) {
			...
			throw new UserIdendifyNotMatchException(member, "아이디 또는 비밀번호가 일치하지 않습니다.");
		}
		
		if (member.getBlockYn().equalsIgnoreCase("Y")) {
			throw new UserIdendifyNotMatchException(member, "비밀번호를 3회 이상 틀려 아이디가 차단되었습니다. 관리자에게 문의하세요.");
		}
